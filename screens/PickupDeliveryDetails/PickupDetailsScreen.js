...
const PickupDetailsScreen = ({navigation}) => {
  ...
  const SENDER_NAME = localize("sender_name");
  ...
  const [senderName, setSenderName] = useState(pickupDetails.senderName);
  const [phoneNumber, setPhoneNumber] = useState(pickupDetails.phoneNumber);
  const [saveAddress, setSaveAddress] = useState(pickupDetails.saveAddress);
  ...
  const fieldChangeHandler = (newValue, field) => {
    ...
    switch (field) {
      case LOCATION_DETAILS:
        ...
        break;
      case SENDER_NAME:
        if (senderName.length > 0 || newValue.trim() !== "") {
  ...
  const validateFields = () => {
    ...
    if (locationDetails.trim().length < 3) {
      ...
      return false;
    } else if (!nameRegex.test(senderName.trim())) {
      setErrorMessage(localize("sender_name_error"));
      return false;
        ...
        <SafeAreaView style={styles.bottomContainer}>
          <TouchableOpacity activeOpacity={1.0} onPress={Keyboard.dismiss}>
            <PopupView style={styles.bottomView} roundTop={true}>
              <Text style={styles.title}>{localize("pick_up_details")}</Text>
              ...
              <InputField
                ...
                text={senderName}
                onTextChange={newValue => {
                  fieldChangeHandler(newValue, SENDER_NAME);
                }}
                onTextSubmit={() => fieldChangeHandler(senderName, SENDER_NAME)}
                maxLength={25}
                keyboardType="default"
                onEndEditing={fieldEndEditingHandler}
                suffix={
...
const styles = StyleSheet.create({
  ...
  map: {
    ...
    bottom: Dimensions.get("window").height / 1.85,
  },
  backButtonContainer: {
    width: 50,
    height: 50,
