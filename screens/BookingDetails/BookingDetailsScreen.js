...
const BookingDetailsScreen = ({navigation}) => {
  ...
  const bookNowButtonPressHandler = () => {
    ...
    if (selectedRide) {
      ...
      navigation.navigate(routes.BOOKING_REVIEW);
    } else {
    ...
    <View style={styles.screen}>
      ...
      {availableRides?.length > 0 ? (
        <SafeAreaView style={styles.bottomContainer}>
          {noRideSelectionError ? (
            <View style={styles.ride_selection_error}>
              <ErrorText error={noRideSelectionError} />
            </View>
          ) : null}
